...
.data
	...
	maxp:	.asciiz "\nThe max value is: "
	minp:	.asciiz "\nThe min value is: "
	min:    .word, 0x7FFFFFFF	# For minimum; use the largest integer value
...
.text
	...
	jal getMM              # Calculate MIN/MAX
	jal mmPrint            # Print min/max
...
mmPrint:la $a0, minp		# Display min
	li $v0, 4
	syscall
...
mmLoop:	slti $t3, $s1, 1
	...
	slt $t3, $t2, $t1	# If current value is less than current min
	beq $t3, $0, skpMin		# jump to min 
	add $t1, $t2, $0	# Save current value as the min
	
skpMin:	slt $t3, $t0, $t2	# elseif current value is greater than max
	beq $t3, $0, next		# jump to max
	add $t0, $t2, $0
